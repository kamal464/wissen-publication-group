...
const baseUrlInterceptor = api.interceptors.request.use((config) => {
  ...
  if (config.url) {
    ...
    if (config.url.startsWith('/') && config.baseURL && config.baseURL.endsWith('/api')) {
      // Keep the leading slash - it's correct
...
api.interceptors.response.use(
  ...
  (error) => {
    // Handle network/connection errors (syscall: 'read' errors)
